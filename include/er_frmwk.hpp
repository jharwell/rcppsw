...
class er_frmwk : public threadable {
 public:
  struct erf_msg {
    explicit erf_msg(const boost::uuids::uuid& id, const erf_lvl::value& lvl,
  ...
  status_t mod_dbglvl(const boost::uuids::uuid& id, const erf_lvl::value& lvl);
  status_t mod_loglvl(const boost::uuids::uuid& id, const erf_lvl::value& lvl);
  int flush(void);
  ...
  void dbglvl(const erf_lvl::value& lvl) { dbglvl_dflt_ = lvl; }
