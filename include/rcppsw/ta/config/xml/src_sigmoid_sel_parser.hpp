...
class src_sigmoid_sel_parser : public rcppsw::config::xml::xml_config_parser {
 public:
  explicit src_sigmoid_sel_parser(uint level)
  ...
  std::shared_ptr<src_sigmoid_sel_config> config_get(void) const {
    return m_config;
  }
 ...
 private:
  ...
  /* clang-format off */
  std::shared_ptr<src_sigmoid_sel_config> m_config{nullptr};
